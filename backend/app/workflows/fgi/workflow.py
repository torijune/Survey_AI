...
from ...domain.fgi.use_cases import FGIAnalysisUseCase
from ...domain.fgi.services import FGIAnalysisService
...
class FGIWorkflowCompatibility:
    ...
    def extract_guide_subjects_rule_based(self, text: str) -> list:
        """기존 rule-based 메서드 호환성"""
        # 이 메서드는 기존 코드와의 호환성을 위해 유지
        # 실제로는 LLM 기반 추출을 사용하는 것이 권장됨
        return self.fgi_service.extract_guide_subjects_rule_based(text.encode('utf-8')) 